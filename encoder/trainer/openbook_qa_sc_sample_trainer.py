...
from encoder.utils.config import OpenBookQASingleChoiceSampleTrainConfig, fix_missing
from encoder.utils.settings import preprocess_cache_dir
from encoder.utils.adafactor import Adafactor


class OpenBookQASingleChoiceSampleTrainer(pl.LightningModule):
    def __init__(
        self,
        config: OpenBookQASingleChoiceSampleTrainConfig,
        stage_result_path="./",
        ...
        self.dataset = OpenBookQABaseDataset(
            tokenizer=None,
        ...
        self.train_reward_predictor_dataset = RewardPredictorSingleChoiceDataset(
            ...
            [
                (
                    d["id"],
                    d["text_question"] + " Context: " + ", ".join(d["choices"]),
                    d["text_answer"],
                    d["choices"],
                    d["original_facts"],
                    d["choice_masks"],
                )
                for d in getattr(self.dataset, f"original_train_data")
            ],
    ...
    def test_dataloader(self):
        path = os.path.join(preprocess_cache_dir, "openbook_qa_sample_result_sc.json")
    ...
    def test_on_every_process(
        ...
        if not self.is_distributed or get_rank() == 0:
            ...
            with open(
                os.path.join(preprocess_cache_dir, "openbook_qa_sample_result_sc.json"),
                "w",
            ) as file:
    ...
    def create_sample_inference_dataloader(self, split):
        return DataLoader(
            dataset=RewardPredictorSingleChoiceBestFirstBeamSearchDatasetWithLimitedNodes(
                ...
                state_delimiter=self.config.state_delimeter,
                end_of_reasoning=self.config.end_of_reasoning,
                wrong_choice=self.config.wrong_choice,
                prune_low_probability_edges=False,
            ),
            batch_size=1,
            collate_fn=lambda x: x,
