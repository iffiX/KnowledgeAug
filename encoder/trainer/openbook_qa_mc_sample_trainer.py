...
from encoder.utils.config import OpenBookQAMultipleChoiceSampleTrainConfig, fix_missing
from encoder.utils.settings import preprocess_cache_dir
from encoder.utils.adafactor import Adafactor


class OpenBookQAMultipleChoiceSampleTrainer(pl.LightningModule):
    def __init__(
        self,
        config: OpenBookQAMultipleChoiceSampleTrainConfig,
        stage_result_path="./",
    ...
    def test_dataloader(self):
        path = os.path.join(preprocess_cache_dir, "openbook_qa_sample_result_mc.json")
    ...
    def test_on_every_process(
        ...
        if not self.is_distributed or get_rank() == 0:
            ...
            with open(
                os.path.join(preprocess_cache_dir, "openbook_qa_sample_result_mc.json"),
                "w",
            ) as file:
